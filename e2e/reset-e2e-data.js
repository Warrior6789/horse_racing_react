...
async function main() {
  ...
  await client.query(
    `UPDATE "Races" SET "Status" = 'Scheduled', "EndTime" = NULL, "PrizePool" = 3000000, "StartTime" = NOW() + interval '7 days' WHERE "RaceID" = $1`,
    [RACE_ID]
  )
  ...
  if (Number(ownerProfileCount.rows[0].count) === 0) {
    await client.query(
      `INSERT INTO "UserProfiles" ("ProfileID", "AccountID", "FullName", "Balance", "IsDeleted", "CreateAt") VALUES (gen_random_uuid(), $1, 'E2E Owner', 0, false, NOW())`,
      [OWNER_ID]
    )
  } else {
    await client.query(`UPDATE "UserProfiles" SET "Balance" = 0 WHERE "AccountID" = $1`, [OWNER_ID])
  }
