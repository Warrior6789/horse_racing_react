...
function PoolCard({ label, total, count, icon, iconColor, bgIcon }) {
  return (
    <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
      <div>
        <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{label}</p>
        <p className="text-2xl font-extrabold text-gray-900">{(total ?? 0).toLocaleString('vi-VN')}</p>
        <p className="text-xs text-gray-400 mt-0.5">{count ?? 0} bets · VND</p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{race?.racecourseName || '—'}</p>
          </div>
        </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {POOL_CARDS.map(({ label, icon, iconColor, bgIcon }) => {
                  ...
                  return <PoolCard key={label} label={label} total={p.totalAmount} count={p.betCount} icon={icon} iconColor={iconColor} bgIcon={bgIcon} />
                })}
              </div>
